...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 05 10:26:53 2022
...
# Logix
#------------------------------------------------------------------------------
def get_PLC_status(cmd, IPAddr = '192.168.1.1'):
    with PLC() as com:
        com.IPAddr = IPAddr
        ret = com.Read(cmd)
    return ret.TagName, ret.Status, ret.Value
...
if __name__ == "__main__":
    # PLC()
    results = get_PLC_status('Started', IPAddr = '192.168.1.1')
    if results: 
        print(f"TagName: {results[0]}")
        print(f"TStatus: {results[1]}")
        print(f"Value: {results[2]}")
